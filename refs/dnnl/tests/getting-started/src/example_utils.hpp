...
inline int
handle_example_errors(std::initializer_list<dnnl::engine::kind> engine_kinds,
...
inline int handle_example_errors(
  ...
  return handle_example_errors({engine_kind},
                               [&]() { example(engine_kind, argc, argv); });
}

// Same as above, but for functions with signature void(dnnl::engine::kind).
inline int
handle_example_errors(std::function<void(dnnl::engine::kind)> example,
                      dnnl::engine::kind engine_kind) {
  return handle_example_errors({engine_kind}, [&]() { example(engine_kind); });
}
    ...
    if (engine_kind_str == "cpu") {
      ...
      if (dnnl::engine::get_count(dnnl::engine::kind::gpu) == 0) {
        std::cout << "Could not find compatible GPU" << std::endl
                  << "Please run the example with CPU instead" << std::endl;
        exit(1);
      }
...
inline void read_from_dnnl_memory(void *handle, dnnl::memory &mem) {
  ...
  if (eng.get_kind() == dnnl::engine::kind::cpu) {
    ...
    cl_int ret =
        clEnqueueReadBuffer(q, m, CL_TRUE, 0, bytes, handle, 0, NULL, NULL);
    if (ret != CL_SUCCESS)
...
inline void write_to_dnnl_memory(void *handle, dnnl::memory &mem) {
  ...
  if (eng.get_kind() == dnnl::engine::kind::cpu) {
    ...
    cl_int ret =
        clEnqueueWriteBuffer(q, m, CL_TRUE, 0, bytes, handle, 0, NULL, NULL);
    if (ret != CL_SUCCESS)
